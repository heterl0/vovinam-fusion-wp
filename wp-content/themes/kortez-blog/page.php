...
<div id="content" class="site-content">
	<div class="container">
		<section class="wrap-detail-page">
			...
			<div class="row">
				<?php
				$page_sidebar_class = kortez_blog_get_sidebar_class( 'page' );
				if( !kortez_blog_wooCom_is_cart() && !kortez_blog_wooCom_is_checkout() && !kortez_blog_wooCom_is_account_page() ){
					if( !get_theme_mod( 'disable_sidebar_page', true ) ){
						kortez_blog_left_sidebar( $page_sidebar_class[ 'sidebarColumnClass' ] );
					}
				}else{
							...
							if( has_post_thumbnail() ){
								if( get_theme_mod( 'page_feature_image', 'show_in_all_pages' ) == 'show_in_all_pages' || !is_front_page() && get_theme_mod( 'page_feature_image', 'show_in_all_pages' ) == 'disable_in_frontpage' || get_theme_mod( 'page_feature_image', 'show_in_all_pages' ) == 'show_in_frontpage' && is_front_page() ){ ?>
								    <figure class="feature-image single-feature-image">
								        <?php 
								        $render_pages_image_size 	= get_theme_mod( 'render_pages_image_size', 'kortez-blog-1370-550' );
								        kortez_blog_image_size( $render_pages_image_size ); ?>
								    </figure>
								<?php }else{
				...
				if( !kortez_blog_wooCom_is_cart() && !kortez_blog_wooCom_is_checkout() && !kortez_blog_wooCom_is_account_page() ){
					if( !get_theme_mod( 'disable_sidebar_page', true ) ){
						kortez_blog_right_sidebar( $page_sidebar_class[ 'sidebarColumnClass' ] );
					}
				}
				?>
			</div>
